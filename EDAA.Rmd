...
# 
# date2 = dat%>%
#   select(date,transactionRevenue,operatingSystem) %>%
#   group_by(date,operatingSystem) %>%
#   summarise(n = round(log(n()),3),rev = round(log(sum(as.numeric(na.omit(transactionRevenue)))+1),3))
# colnames(date)[1] = 'Date'
# date2 = data.frame(date2)
# 
# date$hover <- with(date, paste('Date: ',Date, '<br>', 'Operating System: ',operatingSystem, '<br>',  "Visits: ", n,'<br>','Revenue: ', rev))
# 
# plot_ly(x = ~date, y = ~rev, data = date2, group = date2$operatingSystem, type = 'scatter', mode = 'lines')%>%
#   layout(title = "Revenues per day (log scale)",
#          xaxis = list(title = "Day"),
#         yaxis = list(title = "Revenue"))
