...
```{r}
## Delete the 
dat <- dat %>% dplyr::select(- transactions, -totalTransactionRevenue, -adwordsClickInfo.isVideoAd, -visitStartTime)
...
loss.logistic_lm <- sum((log(dat.test$transactionRevenue + 1) - pred.res)^2) / length(predicition.index)
...
dat.keras.x <- scale(dat.keras.x)
  
dat.keras.y <- dat %>% dplyr::select(transactionRevenue)

## Dataset for training model
dat.keras.train.x <- dat.keras.x[train.index, ]
dat.keras.train.y <- as.numeric(pull(dat.keras.y[train.index, ]) > 0)
...
model_nn %>% 
  layer_dense(units = 16, activation = "relu", input_shape = ncol(dat.keras.x)) %>% 
  layer_dropout(rate = 0.1) %>% 
  layer_dense(units = 32, activation = "relu") %>%
  layer_dropout(rate = 0.25) %>%
  layer_dense(units = 1, activation = "linear")

model_nn %>% compile(loss = "cross_entropy",
...
## Fitting the lasso 
model_lasso <- cv.glmnet(dat.glmnet.x, (log(pull(dat.glmnet.y) + 1)), family="gaussian", alpha = 0, nlambda = 20, type.measure = "mse")
pred.res <- predict(model_lasso, dat.glmnet.test.x, s=m_glm$lambda.min, type = "response") 
...
print(paste0("The loss for the lasso regression is ", round(loss.xgb,4) ))
